...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/11 12:43:40 by achatzit          #+#    #+#             */
/*   Updated: 2023/12/11 13:07:51 by achatzit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t	starrynight_palette(t_scaled_pixel spx, t_fol *f)
{
	...
	rgb.g = (1 - t2) * rgb.g + t2 * ((BLU >> 8) & 0xFF);
	rgb.b = (1 - t2) * rgb.b + t2 * (BLU & 0xFF);
	final_color = (uint32_t)0 << 24 | (uint32_t)(0) << 16 | \
...
uint32_t	greenery_palette(t_scaled_pixel spx, t_fol *f)
{
	...
	rgb.g = (1 - t2) * rgb.g + t2 * ((BLU >> 8) & 0xFF);
	rgb.b = (1 - t2) * rgb.b + t2 * (BLU & 0xFF);
	final_color = (uint32_t)0 << 24 | (uint32_t)(rgb.g * f->zoom) << 16 \
		| (uint32_t)rgb.b << 8 | 255;
	return (final_color);
}
